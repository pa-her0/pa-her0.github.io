...
| 打开本地工作台 | 双击 `启动博客工作台.command`（macOS）或 `启动博客工作台.cmd`（Windows） | 自动打开仅本机可访问的文章编辑与发布页面 |
...
macOS 双击项目根目录的 `启动博客工作台.command`，Windows 双击
`启动博客工作台.cmd`。工作台会自动在浏览器打开：

```text
http://127.0.0.1:4322/
```

工作台仅监听本机地址，不会作为公开博客页面部署。可以在其中新建和编辑文章、
